...
import {
  getSaikyoDirection,
} from "../utils/routing/getSaikyoDirection";

import {
  resolveLiveJourney,
} from "../utils/routing/resolveLiveJourney";
...
if (!directionId) {
  throw new Error(
    "Failed to resolve Saikyo direction.",
  );
}

const run = async () => {
  console.log(
    "=== SAIKYO LIVE JOURNEY TEST ===",
  );
  ...
  if (result.status !== "resolved") {
    throw new Error(
      "Saikyo Live Journey was not resolved.",
    );
  }
  ...
  if (!segment) {
    throw new Error(
      "Resolved segment is missing.",
    );
  }

  console.log("");
  console.log(
    "PASS: LIVE SAIKYO JOURNEY",
  );
  ...
  console.log(
    "Train:",
    segment.train.candidate.trainNumber,
  );
};
