...
export default function LineScreen() {
   ...
   * ========================================
   *
   * Registry 데이터가 아직 완성되지 않았거나
   * 잘못 연결되어 있어도
   * 앱 전체가 Render Error로 죽지 않게 한다.
   */
  ...
  if (stations.length === 0) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.notFoundContainer}>
          <Text style={styles.notFoundTitle}>역 데이터가 없습니다.</Text>

          <Text style={styles.notFoundDescription}>{line.nameKo}</Text>

          <Text style={styles.debugText}>lineId: {String(lineId)}</Text>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.summaryArea}>
          <Text style={styles.summaryText}>총 {stations.length}개 역</Text>
        </View>
        ...
        <View style={styles.routeContainer}>
          {stations.map((station, index) => {
            ...
            const transfers = Array.isArray(station.transfers)
              ? station.transfers
              : [];
