 ...
 *
 * 막차 영역에서 자정 직후 운행되는 열차를 이어서 비교하되,
 * 03:00 이후의 새벽 첫차는 다음 운행일의 열차로 분리한다.
 *
 * 예:
 * 23:58 -> 1438
 * 00:07 -> 1447
...
const toServiceMinutes = (
  ...
  /*
   * 막차 계산에서 자정 직후의 심야 열차만
   * 전날 서비스의 연장으로 취급한다.
   *
   * 00:xx ~ 02:59 -> 24:xx ~ 26:59
   * 03:00 이후    -> 당일 시각 그대로
   *
   * 이렇게 해야 하루 전체 시간표에 포함된 04~05시 첫차가
   * 전날 23~00시 막차보다 늦은 열차로 잘못 선택되지 않는다.
   */
  if (minutes < 3 * 60) {
